...
namespace Sporeholm.World
{
    ...
    // in flood-fill order.
    //
    // Walls / Doors are treated as boundaries: walls are impassable and
    // don't belong to any room; doors are passable and connect the rooms
    // on either side (so a room with a door inside doesn't get sub-divided
    // — the doorway connects rather than separates).
    //
            ...
            for (int x = 0; x < W; x++)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    switch (slot.Type)
                    {
                        ...
                        case StructureType.Door:         furnitureCount++; furnitureBeauty += qMul; break;   // counts as "object" for beauty
