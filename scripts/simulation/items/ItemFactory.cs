...
namespace Sporeholm.Simulation.Items
{
	...
	// — from a gathered Capberry through a scenario-rolled Masterwork
	...
	//      (a Forager picking a Capberry knows the Plant/Capberry
	...
	public static class ItemFactory
	{
		...
		// (Capberry / SmallMushroom / HerbCluster / MagicBerry). The
		...
		public static (string SubType, MaterialKey Material)? FoodFromVegetation(
			Sporeholm.World.VegetationType v) => v switch
		{
			Sporeholm.World.VegetationType.CapberryBush  => ("Capberry",    new MaterialKey("Plant","Capberry")),
			Sporeholm.World.VegetationType.SmallMushroom   => ("SmallMushroom", new MaterialKey("Plant","SmallMushroom")),
			Sporeholm.World.VegetationType.LargeMushroom   => ("SmallMushroom", new MaterialKey("Plant","LargeMushroom")),
			Sporeholm.World.VegetationType.HerbCluster     => ("HerbCluster",   new MaterialKey("Plant","HerbCluster")),
			Sporeholm.World.VegetationType.MagicFlower     => ("MagicBerry",    new MaterialKey("Plant","MagicBerry")),
			Sporeholm.World.VegetationType.PineShroom      => ("SmallMushroom", new MaterialKey("Plant","SmallMushroom")),
			Sporeholm.World.VegetationType.PalmShroom      => ("SmallMushroom", new MaterialKey("Plant","LargeMushroom")),
			Sporeholm.World.VegetationType.SmallSandshroom => ("SmallMushroom", new MaterialKey("Plant","SmallMushroom")),
			Sporeholm.World.VegetationType.LargeSandshroom => ("SmallMushroom", new MaterialKey("Plant","LargeMushroom")),
			_ => null,
		};
		...
		public static bool VegetationYieldsMagicEssence(Sporeholm.World.VegetationType v) =>
			v == Sporeholm.World.VegetationType.MagicFlower;
		...
		// (producing Cuttings).
		public static MaterialKey WoodFromVegetation(Sporeholm.World.VegetationType v) => v switch
		{
			Sporeholm.World.VegetationType.LargeMushroom   => new MaterialKey("Wood","Fungal"),
			Sporeholm.World.VegetationType.LargeSandshroom => new MaterialKey("Wood","Fungal"),
			Sporeholm.World.VegetationType.PalmShroom      => new MaterialKey("Wood","Fungal"),
		...
		public static (ItemKind Kind, string SubType, MaterialKey Material) MaterialFromTerrain(
			Sporeholm.World.TerrainType t, Random rng) => t switch
		{
			Sporeholm.World.TerrainType.Boulder    =>
				(ItemKind.Material, "StoneBlock",
				 MaterialRegistry.RollInFamily("Stone", rng)?.Key ?? new MaterialKey("Stone","Granite")),
			Sporeholm.World.TerrainType.DeadLog    =>
				(ItemKind.Material, "WoodLog",   new MaterialKey("Wood","DeadWood")),
			Sporeholm.World.TerrainType.LivingWood =>
		...
		// the original plant's BaseYield so cutting a CapberryBush
		...
		public static (string SubType, MaterialKey Material, int Quantity) CuttingsFromVegetation(
			Sporeholm.World.VegetationType v) => v switch
		{
			Sporeholm.World.VegetationType.LargeMushroom   => ("Cuttings", new MaterialKey("Plant","Cuttings"), 3),
			Sporeholm.World.VegetationType.LargeSandshroom => ("Cuttings", new MaterialKey("Plant","Cuttings"), 3),
			Sporeholm.World.VegetationType.PalmShroom      => ("Cuttings", new MaterialKey("Plant","Cuttings"), 2),
			Sporeholm.World.VegetationType.CapberryBush  => ("Cuttings", new MaterialKey("Plant","Cuttings"), 2),
			Sporeholm.World.VegetationType.HerbCluster     => ("Cuttings", new MaterialKey("Plant","Cuttings"), 1),
			_ => ("Cuttings", new MaterialKey("Plant","Cuttings"), 1),
		};

		// v0.3.47 (Phase 4 sub-B) — role-based starting kit.
		//
		// Rolls a small bundle of starting items per shroomp, sized for ~1 week
		...
		// patch; for v0.3.47 every shroomp gets a balanced kit.
		public static List<Item> RollStartingKit(string role, Random rng, long globalTick)
		{
			...
			string[] foodPool = { "Capberry", "SmallMushroom", "HerbCluster", "MagicBerry" };
			for (int i = 0; i < 4; i++)
			{
			...
			// shroomp without changing the base kit.
			switch (role)
			{
				case "Guardian":
