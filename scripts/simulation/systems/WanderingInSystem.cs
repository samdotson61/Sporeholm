...
namespace Sporeholm.Simulation.Systems
{
    ...
    //
    // A new shroomp shows up at the colony edge asking to join. Roadmap §4
    ...
    //     ceases past 200.
    //   • Sex ratio — every wanderer rolls against 1:49 ratio (Shroomps canon).
    ...
    public static class WanderingInSystem
    {
        ...
        // Roll a wandering-in event for this season. Returns a new Shroomp
        ...
        public static Shroomp? TryWanderer(IReadOnlyList<Shroomp> living, Random rng, int foodTotal, long globalTick = 0)
        {
            ...
            // (≤ 5 shroomps) so the founding can still attract a female.
            int floor = Math.Max(1, alive) * 15;   // 15 food/shroomp = "fed"
            if (alive > 5 && foodTotal < floor) return null;

            // 1:49 female ratio (canonical Shroomps distribution).
            ...
            // a vocation).
            string[] roles = { "Forager", "Crafter", "Scholar", "Sage",
                               "Caretaker", "Guardian" };
            ...
            // doesn't appear at (0,0). Picks a random alive shroomp as the
            ...
            // straight into the shroomp's dominant hand.
            if (rng.NextDouble() < 0.60)
            {
                ...
                var tool = Sporeholm.Simulation.Items.ItemFactory.Create(
                    Sporeholm.Simulation.Items.ItemKind.Tool, toolSub,
                    material: null, rng, globalTick, skillLevel: 0, quantity: 1);
                tool.OwnerShroompId = s.Id;
