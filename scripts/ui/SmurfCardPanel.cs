...
// Floating "wanted poster" style Smurf Card with left-side tab strip.
// Tabs: Main (needs, mood, personality) · Health (biological traits, body parts).
public partial class SmurfCardPanel : Control
{
    [Signal] public delegate void RoleChangeRequestedEventHandler(string smurfName, string newRole);
    ...
    private static readonly string[] Roles =
        { "Forager", "Crafter", "Scholar", "Mage", "Caretaker", "Guardian", "Elder", "Unassigned" };
    ...
    public SmurfulationC.SimulationManager? Sim { get; set; }
    ...
    public override void _Ready()
    {
        ...
        // below the TileInfoOverlay at 720p screens. With the order bar
        // reserving ~240 px from the bottom and TileInfoOverlay claiming
        // ~155 px from the top, a 720 px screen has only ~325 px of
        // vertical space for the card. 320 leaves a 5 px clearance.
        // Content beyond 320 px scrolls inside the card's existing
        // ScrollContainer (no clipping).
    ...
    //
    // Three equipment slots (Tool / Weapon / Apparel) plus the smurf's
    ...
    // Drop (drops the item on the smurf's current tile via
    ...
    private VBoxContainer BuildInventoryTab()
    {
        ...
        // so the player sees how much haul room the smurf still has.
    ...
    private void RefreshInventoryTab(SmurfSnapshot snap)
    {
        ...
        foreach (var slot in SmurfulationC.Simulation.Items.EquipSlotMeta.All)
        {
            ...
            bool isDominant =
                (slot == SmurfulationC.Simulation.Items.EquipSlot.LeftHand  && snap.Handedness == Handedness.Left)
             || (slot == SmurfulationC.Simulation.Items.EquipSlot.RightHand && snap.Handedness == Handedness.Right);
        ...
        // v0.4.30 — list every stack in the smurf's Inventory rather than
        ...
        if (snap.InventoryItems != null && snap.InventoryItems.Count > 0)
        {
            for (int i = 0; i < snap.InventoryItems.Count; i++)
            {
                ...
                var defSub = SmurfulationC.Simulation.Items.ItemRegistry.Get(
                    System.Enum.TryParse<SmurfulationC.Simulation.Items.ItemKind>(entry.Kind, out var ek)
                        ? ek : SmurfulationC.Simulation.Items.ItemKind.Material,
                    entry.SubType);
    ...
    private void AddSlotRow(SmurfulationC.Simulation.Items.EquipSlot slot,
        string? equippedSub, string? kindStr, bool isDominant, string smurfName)
    {
        var row = new HBoxContainer();
        row.AddThemeConstantOverride("separation", 6);
        _inventorySlotsBox.AddChild(row);

        string slotDisplay = SmurfulationC.Simulation.Items.EquipSlotMeta.Display(slot);
        if (isDominant) slotDisplay = "✋ " + slotDisplay;
        var slotLbl = MakeLabel(slotDisplay, 10, Gold);
        slotLbl.CustomMinimumSize = new Vector2(72, 0);
        row.AddChild(slotLbl);

        string display = "(empty)";
        if (equippedSub != null && kindStr != null
            && System.Enum.TryParse<SmurfulationC.Simulation.Items.ItemKind>(kindStr, out var kind))
        {
            var def = SmurfulationC.Simulation.Items.ItemRegistry.Get(kind, equippedSub);
        ...
        if (equippedSub != null)
        {
            row.AddChild(MakeSmallActionBtn("Unequip", () => Sim?.RequestUnequip(smurfName, slotName)));
            row.AddChild(MakeSmallActionBtn("Drop",    () => Sim?.RequestDropEquipped(smurfName, slotName)));
        }
        else
        {
            row.AddChild(MakeSmallActionBtn("Equip", () => OpenEquipPickerForSlot(smurfName, slot)));
        }
    }
    ...
    private Button MakeSmallActionBtn(string text, System.Action onPressed)
    {
        ...
        {
            Text              = text,
            CustomMinimumSize = new Vector2(52, 20),
            FocusMode         = FocusModeEnum.None,
        };
        btn.AddThemeFontSizeOverride("font_size", 9);
        btn.AddThemeStyleboxOverride("normal",  FloatingPanelStyle.MakeToolbarButton(false));
    ...
    private void OpenEquipPickerForSlot(string smurfName, SmurfulationC.Simulation.Items.EquipSlot slot)
    {
        if (Sim == null) return;

        // Figure out which BodyClass the slot accepts.
        var targetClass = SmurfulationC.Simulation.Items.EquipSlotMeta.BodyClass.None;
        foreach (var bc in System.Enum.GetValues<SmurfulationC.Simulation.Items.EquipSlotMeta.BodyClass>())
        {
            foreach (var s in SmurfulationC.Simulation.Items.EquipSlotMeta.SlotsFor(bc))
            {
                if (s == slot) { targetClass = bc; break; }
            }
            if (targetClass != SmurfulationC.Simulation.Items.EquipSlotMeta.BodyClass.None) break;
        }
        if (targetClass == SmurfulationC.Simulation.Items.EquipSlotMeta.BodyClass.None) return;

        var inv = Sim.GetInventorySnapshot();
        var matches = new System.Collections.Generic.List<SmurfulationC.Simulation.Items.InventoryRow>();
        foreach (var row in inv)
        {
            var def = SmurfulationC.Simulation.Items.ItemRegistry.Get(row.Kind, row.SubType);
            if (def == null) continue;
            if (def.BodyClass == targetClass) matches.Add(row);
        }

        var popup = new PopupPanel { Title = $"Equip — {SmurfulationC.Simulation.Items.EquipSlotMeta.Display(slot)}" };
        ...
        else
        {
            ...
            {
                var def = SmurfulationC.Simulation.Items.ItemRegistry.Get(m.Kind, m.SubType);
                var matDef = SmurfulationC.Simulation.Items.MaterialRegistry.Get(
                    new SmurfulationC.Simulation.Items.MaterialKey(m.MaterialFamily, m.MaterialSubType));
                ...
                {
                    Sim.RequestEquip(smurfName, m.SubType, m.MaterialFamily, m.MaterialSubType, slotHint);
                    popup.Hide();
                    popup.QueueFree();
                });
    ...
    private VBoxContainer BuildMoodTab()
    {
        ...
        // can see at a glance what the smurf's recent experiences add up
    ...
    private void ApplyTooltips()
    {
        ...
        _closeBtn.TooltipText  = "Close smurf card";
        ...
        _moodStateDesc.TooltipText = "Description of the smurf's current mental state.";
    ...
    public void Refresh(IReadOnlyList<SmurfSnapshot> snaps)
    {
    ...
    {
        if (!Visible || _selectedName == null) return;
        if (snap.SmurfsByName.TryGetValue(_selectedName, out var ss))
        {
    ...
    // ── Refresh ───────────────────────────────────────────────────────────────
    private void RefreshFromSnapshot(SmurfSnapshot snap)
    {
        // v0.3.46 — show the smurf's current activity beside their name so
    ...
    private void RefreshMoodTab(SmurfSnapshot snap)
    {
