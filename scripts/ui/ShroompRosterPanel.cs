...
// v0.3.27 — RimWorld-style roster table for the Shroomps tab. Each row shows
// one alive shroomp with columns for the data the player needs to scan a
// colony at a glance:
//
...
//   • Name              — clickable label, selects shroomp + opens unit card.
...
// omitted because none of those systems exist in Sporeholm yet — they'd
// add empty columns the player couldn't interact with. Per-role Work
// priorities live in their own ⚙ Jobs tab (stubbed).
public partial class ShroompRosterPanel : Control
{
    [Signal] public delegate void ShroompZoomRequestedEventHandler(string name);
    [Signal] public delegate void ShroompSelectRequestedEventHandler(string name);
    ...
    private void BuildContent()
    {
        ...
        // Header row — column titles aligned with the per-shroomp rows.
        ...
        // Scrollable list of shroomp rows.
    ...
    public void Refresh(IReadOnlyList<SimulationC_Roster_Row> rows)
    {
        ...
        // Drop rows for shroomps that no longer exist in the snapshot.
    ...
    private RosterRowCtrls BuildRow(SimulationC_Roster_Row r)
    {
        ...
        var focus = new Button
        {
            ...
            TooltipText       = "Centre the camera on this shroomp",
            Flat              = true,
        };
        focus.AddThemeFontSizeOverride("font_size", UITheme.Scaled(12));
        focus.Pressed += () => EmitSignal(SignalName.ShroompZoomRequested, r.Name);
        row.AddChild(focus);
        ...
        nameBtn.GuiInput += ev =>
        {
            ...
            {
                if (mb.DoubleClick)
                    EmitSignal(SignalName.ShroompZoomRequested, r.Name);
                else
                    EmitSignal(SignalName.ShroompSelectRequested, r.Name);
            }
        };
    ...
    private static void SetNeedBar(ProgressBar bar, float v)
    {
        ...
        // for any red/amber pip and knows that shroomp needs attention.
        Color c;
...
// + combat status alongside name/role/mood. Populated from ShroompSnapshot in
// GameController.OnTick.
public readonly record struct SimulationC_Roster_Row(
    string                              Name,
    string                              Role,
    Sporeholm.Simulation.MoodState  MoodState,
    float                               Nutrition,
    float                               Rest,
    float                               Social,
    float                               Magic,
    float                               Safety,
    Sporeholm.Simulation.TaskType   CurrentTask,
